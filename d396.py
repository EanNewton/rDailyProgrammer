def p396(n: int, h: int) -> int:
    ...
    The marketing division is working on a slogan for the latest model:
    "Able to survive a K-meter drop!".
    They just need to know the largest possible whole number value of K they can truthfully claim.
    Someone has already dropped one from 101 meters up and it broke, so they know the largest possible value is
    somewhere between 0 and 100 inclusive.

    Here's where you come in.
    You must find the value of K such that a phone will not break if dropped from K meters,
    but will break if dropped from K+1 meters.
    For the purpose of this challenge, these tests are completely reliable,
    so a single test at both K and K+1 meters is enough to establish this.
    Also, as long as a phone survives the drop, it suffers no damage whatsoever and can be reused in subsequent tests.
    ...
    so assuming each trial takes the same amount of time, you need to know the maximum number of trials you'll need,
...
def p396b1(n: int, h=123456789) -> bool:
    """
...
def test() -> bool:
    assert p396(1, 100) == 100
    assert p396(2, 100) == 14
    assert p396(3, 100) == 9
    assert p396(1, 1) == 1
    assert p396(2, 456) == 30
    assert p396(3, 456) == 14
    assert p396(4, 456) == 11
    assert p396(2, 789) == 40
    assert p396(3, 789) == 17
    assert p396(4, 789) == 12
    return True